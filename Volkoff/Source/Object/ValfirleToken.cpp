...
bool ValfirleToken::Initialize( const CreateData& data_ )
{
	...
	case WEAPON_BOSS_KATANA:
		...
		this->mCharaPanel->SetColorScale(ColorF(0.25f,0.80f,0.25f));
		break;

	case WEAPON_BOSS_TAEKWONDO:
		this->m_pszName = "花の??";
		r = Character::Initialize();
		this->mCharaPanel->SetColorScale(ColorF(0.95f,0.7f,0.7f));
		break;

	case WEAPON_BOSS_KNIFE:
		this->m_pszName = "三角木馬の??";
		r = Character::Initialize();
		this->mCharaPanel->SetColorScale(ColorF(0.7f,0.7f,0.95f));
		break;
