...
bool CameraManager::Initialize()
{
	...
	//Accessor::Main3Camera->setPosition(v);
 //   v.z = 0;
	//Accessor::Main3Camera->setLookAt(v);

	this->m_eMode = CAMERA_UNDEF;
	this->m_pCLookObj = NULL;
    
    Accessor::Main3Camera->setFovY(Math::PI * 0.125);
...
void CameraManager::Update()
{
	...
	case CAMERA_MOVE:
		v = Accessor::Main3Camera->position();
		v += this->m_CVector;
		Accessor::Main3Camera->setPosition(v);
		v.z = 0;
		Accessor::Main3Camera->lookAt(v);
		break;
...
void CameraManager::Move(LVector3 pos_,LVector3 vector_)
{
	...
	Accessor::Main3Camera->setPosition(pos_);
