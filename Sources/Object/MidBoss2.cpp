...
// ● オブジェクトを作成する
//---------------------------------------------------------------------------
MidBoss2* MidBoss2::create( void* args_, bool is_entity_ )
{
    MidBoss2* o = NEW MidBoss2();
...
// ● オブジェクトをランダムで作成する
//---------------------------------------------------------------------------
MidBoss2* MidBoss2::createRandom()
{
	...
	MidBoss2* pEnemy = MidBoss2::create( &data );
...
*/
//---------------------------------------------------------------------------
bool MidBoss2::Initialize( const CreateData& data_ )
{
	bool r = false;
	...
	case WEAPON_BOSS_SCYTHE:
		this->m_pszName = "野菜洗浄機の細田";
		r = Character::Initialize();
		this->mCharaPanel->setColorScale(Color(0.0f,0.0f,0.0f));
		break;
	default:
		this->m_pszName = "眠りを売る山本";
		r = Character::Initialize();
	...
	this->mLife = this->mMaxLife = scg_nMidBoss2Life[GameManager::getInstance()->getRank()];
...
bool MidBoss2::UpdateSelf()
{
	...
	case ENEMY_ACT_CHASE:
		...
		if(this->SearchPlayer())
		{
			...
			//まれにジャンプする
			if(!(::rand() % scg_nMidBoss2JumpRatio[GameManager::getInstance()->getRank()]))
			{
				button_B = true;
			}
			...
			{
				//ランダムで観察を開始する
				if(!(::rand() % scg_nMidBoss2StandbyRatio[GameManager::getInstance()->getRank()]))
				{
	...
	case ENEMY_ACT_ATTACK_RANGE:
		...
		{
			//ランダムで攻撃を開始する
			if(!(::rand() % scg_nMidBoss2AttackRatio[GameManager::getInstance()->getRank()]))
			{
				this->m_eAction = ENEMY_ACT_ATTACK;
				break;
			}
			//ランダムで防御を開始する
			if(!(::rand() % scg_nMidBoss2GuardRatio[GameManager::getInstance()->getRank()]))
			{
			...
			//ランダムでしゃがみガードを開始する
			if(!(::rand() % scg_nMidBoss2SquatGuardRatio[GameManager::getInstance()->getRank()]))
			{
				this->m_nGuardCnt = ::rand() % this->scm_nGuardIntv;
			...
			//ランダムでしゃがみを開始する
			if(!(::rand() % scg_nMidBoss2SquatRatio[GameManager::getInstance()->getRank()]))
			{
			...
			//ランダムで逃走を開始する
			if(!(::rand() % scg_nMidBoss2EscapeRatio[GameManager::getInstance()->getRank()]))
			{
	...
	case ENEMY_ACT_ATTACK:
		if(this->SearchAttack1Range())
		{
			...
			case CHARASTATE_ATTACK_1:
				//通常攻撃2段目
				if(scg_bMidBoss2Attack2Permission[GameManager::getInstance()->getRank()] && (::rand() % scg_nMidBoss2Attack2Ratio[GameManager::getInstance()->getRank()]))
					button_A = true;
				else
					this->m_eAction = ENEMY_ACT_WAIT;
				break;
			...
			case CHARASTATE_ATTACK_2:
				//通常攻撃3段目
				if(scg_bMidBoss2Attack3Permission[GameManager::getInstance()->getRank()] && (::rand() % scg_nMidBoss2Attack3Ratio[GameManager::getInstance()->getRank()]))
					button_A = true;
				else
					this->m_eAction = ENEMY_ACT_WAIT;
				break;
			...
			default:
				//ジャンプ攻撃は事前にジャンプする
				if(!(::rand() % scg_nMidBoss2JumpAttackRatio[GameManager::getInstance()->getRank()]))
				{
					button_B = true;
					break;
				}
				//しゃがみ攻撃
				if(!(::rand() % scg_nMidBoss2SquatAttackRatio[GameManager::getInstance()->getRank()]))
				{
					Down = true;
				}
    ...
    if ( mStunFrame == 0 )
    {
        ...
        if ( isAttacking() )
        {
			...
			if ( !mAttackLocked )
			{
				...
				if ( button_A )
				{
					...
					{
					case CHARASTATE_ATTACK_1:
						if(scg_bMidBoss2Attack2Permission[GameManager::getInstance()->getRank()])
							getManager()->postEvent( getHandle(), EV_ACTION_ATTACK_2, NULL, 0 );
						break;

					case CHARASTATE_ATTACK_2:
						if(scg_bMidBoss2Attack3Permission[GameManager::getInstance()->getRank()])
        ...
        else
        {
            ...
            if ( button_A && m_eCharaState != EV_ACTION_ATTACK_1 )  // 後ろのは仮
            {
                ...
                {
					// 攻撃のため行動制限
					if(scg_bMidBoss2Attack1Permission[GameManager::getInstance()->getRank()])
					{
						this->AttackLock();
...
*/
//---------------------------------------------------------------------------
int MidBoss2::HandleEvent(u32 event_,void *args_)
{
	///////////////////////////////////////// 戦闘不能開始
	switch ( event_ )
	{
...
//---------------------------------------------------------------------------
/*! 
	@brief MidBoss2::SearchAttack1Range
...
*/
//---------------------------------------------------------------------------
bool MidBoss2::SearchAttack1Range()
{
	bool bRes = false;
	LRect *r = NULL;
	Player *pPlayer = GameManager::getInstance()->getPlayer();
	...
	//攻撃範囲内
	if(::abs(this->mPosition.x - pPlayer->getPosition().x) <= scg_nMidBoss2AttackRange[GameManager::getInstance()->getRank()]) //40は一時的
	{
		bRes = true;
	}
